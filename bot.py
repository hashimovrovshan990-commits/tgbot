...
class Database:
    ...
    def update_trade_field(self, trade_id: int, user_id: int, field: str, value: Union[str, float, None]):
...
LANG = {
    "ru": {
        ...
        "pnl": "P/L",
        "enter_pnl": "💰 Введите фактический результат сделки в USD (например, 50 для прибыли или -30 для убытка):",
        "pnl_updated": "✅ P/L обновлён!",
        "feedback_prompt": "📝 Напишите ваше сообщение, и оно будет отправлено администратору.",
        "feedback_sent": "✅ Сообщение отправлено. Спасибо!",
        "feedback_unavailable": "❌ Функция обратной связи временно недоступна.",
        "faq": """
📌 **Часто задаваемые вопросы**

❓ **Как указать результат сделки (P/L)?**  
После выбора статуса «В прибыли» или «В убытке» бот автоматически запросит ввод фактической прибыли/убытка. Если вы пропустили этот шаг, можете отредактировать сделку позже через «✍️ Изменить».

❓ **Сколько сделок можно добавить бесплатно?**  
Бесплатно доступно до 20 сделок. Для снятия лимита необходимо оформить подписку.

❓ **Как работает подписка?**  
Подписка оплачивается Telegram Stars. После оплаты премиум-доступ активируется на выбранный срок (30 или 90 дней). Автопродления нет – по истечении срока вы получите предложение продлить подписку.

❓ **Что делать, если после оплаты подписка не активировалась?**  
Обычно активация происходит мгновенно. Если этого не произошло, напишите в `/feedback` – мы разберёмся.

❓ **Как сменить язык?**  
Перейдите в «🧰 Настройки» и нажмите «🌍 Выбрать язык».

❓ **Можно ли экспортировать данные?**  
Да, через меню «📤 Экспорт» вы можете скачать Excel-файл со всеми сделками по выбранному счёту.

❓ **Что такое чеклисты?**  
Чеклисты помогают фиксировать условия входа в сделку. Вы можете создать свой чеклист или использовать готовые шаблоны.

❓ **Как связаться с разработчиком?**  
Используйте команду /feedback, чтобы отправить сообщение администратору.
        """,
    },
    "en": {
        ...
        "pnl": "P/L",
        "enter_pnl": "💰 Enter actual trade result in USD (e.g., 50 for profit or -30 for loss):",
        "pnl_updated": "✅ P/L updated!",
        "feedback_prompt": "📝 Write your message, it will be sent to the administrator.",
        "feedback_sent": "✅ Message sent. Thank you!",
        "feedback_unavailable": "❌ Feedback function is temporarily unavailable.",
        "faq": """
📌 **Frequently Asked Questions**

❓ **How do I set the trade result (P/L)?**  
After selecting status "In profit" or "In loss", the bot will automatically ask for the actual profit/loss. If you skipped this step, you can edit the trade later via "✍️ Edit".

❓ **How many trades can I add for free?**  
Up to 20 trades are free. To remove the limit, you need to purchase a subscription.

❓ **How does subscription work?**  
Subscription is paid with Telegram Stars. After payment, premium access is activated for the chosen period (30 or 90 days). No auto-renewal – after expiration you'll be offered to renew.

❓ **What if subscription doesn't activate after payment?**  
Activation is usually instant. If not, use /feedback to contact support.

❓ **How to change language?**  
Go to "🧰 Settings" and click "🌍 Select language".

❓ **Can I export data?**  
Yes, via "📤 Export" you can download an Excel file with all trades for a selected account.

❓ **What are checklists?**  
Checklists help you record entry conditions. You can create your own or use templates.

❓ **How to contact the developer?**  
Use /feedback command to send a message to the administrator.
        """,
    }
}
...
async def select_st(call: types.CallbackQuery, state: FSMContext):
    ...
    if st in ("PROFIT", "LOSS"):
        await call.message.edit_text(
            f"💰 {get_text(user_id, 'enter_pnl')}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_text(user_id, "skip"), callback_data=CALLBACK_SKIP)]
            ])
        )
        await state.set_state(States.trade_step_pnl)
    else:
        await call.message.edit_text(
...
# ---------- Ввод PnL ----------
@dp.message(States.trade_step_pnl)
async def step_pnl(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if not message.text:
        return
    text = message.text.strip()
...
async def show_confirmation(message: types.Message, state: FSMContext):
    ...
    pnl_text = f"{data.get('pnl', 'не указан')} USD" if data.get('pnl') is not None else "не указан"
...
async def save_trade(call: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    try:
        trade_id = db.add_trade(user_id, data)
...
async def skip_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    if curr_state == States.trade_step_2.state:
        ...
        await state.set_state(States.trade_step_9)
    elif curr_state == States.trade_step_pnl.state:
        await state.update_data(pnl=None)
        await call.message.edit_text(
            f"8️⃣ {get_text(user_id, 'enter_strategy')}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_text(user_id, "skip"), callback_data=CALLBACK_SKIP)]
...
async def feedback_cmd(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if ADMIN_ID == 0:
        await message.answer(get_text(user_id, "feedback_unavailable"))
        return
    await state.set_state(States.feedback_wait)
    await message.answer(get_text(user_id, "feedback_prompt"))

@dp.message(States.feedback_wait)
async def feedback_handle(message: types.Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            ADMIN_ID,
            f"📬 Сообщение от пользователя {user_id} (@{message.from_user.username}):\n\n{text}"
        )
